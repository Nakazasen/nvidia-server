...
function getAgentTools() {
    return [
        ...
        },
        {
            ...
            function: {
                name: 'move_file',
                description: 'Propose move/rename of a single existing workspace file to another workspace-relative path. This creates a pending move operation that must be reviewed/applied.',
                parameters: {
                    type: 'object',
                    properties: {
                        sourcePath: { type: 'string', description: 'Workspace-relative source file path.' },
                        targetPath: { type: 'string', description: 'Workspace-relative target file path.' },
                        reason: { type: 'string' }
                    },
...
async function runToolCall(toolCall, options = {}) {
    ...
    try {
        ...
        if (DESTRUCTIVE_TOOLS.has(name) && !options.allowDestructive) {
            return {
                ...
                approvalRequest: {
                    tool: name,
                    args,
                    targetPath: typeof args.filePath === 'string' ? args.filePath : (typeof args.targetPath === 'string' ? args.targetPath : undefined),
                    summary: typeof args.filePath === 'string'
                        ? `Approve ${name} for workspace path "${args.filePath}".`
                        : (typeof args.sourcePath === 'string' && typeof args.targetPath === 'string')
                            ? `Approve ${name} from "${args.sourcePath}" to "${args.targetPath}".`
        ...
        if (['project_indexer', 'semantic_index', 'index_status', 'index_build', 'index_refresh', 'index_search', 'git_context', 'git_status', 'git_diff', 'git_file_diff', 'git_log', 'git_stage', 'git_unstage', 'git_discard', 'git_commit_draft', 'list_dir', 'read_file', 'read_file_paged', 'write_file', 'delete_file', 'move_file', 'apply_patch', 'apply_pending_edit', 'discard_pending_edit', 'search_files', 'search', 'execute_command', 'start_command_job', 'command_job_status', 'cancel_command_job'].includes(name)) {
...
function buildAbwBridgePayload(result) {
    ...
    return {
        ...
        evidenceTier: data.knowledge_evidence_tier ?? null,
        sources: Array.isArray(data.sources) ? data.sources : [],
        warnings: Array.isArray(data.warnings) ? data.warnings : [],
        gapLogged: data.gap_logged === true,
        gapLogSuppressed: data.gap_log_suppressed === true,
        wouldLogGap: data.would_log_gap === true,
        runtimeWriteSuppressed: data.runtime_write_suppressed === true,
...
async function routeApiTool(toolName, args) {
    if (['list_dir', 'read_file', 'read_file_paged', 'write_file', 'delete_file', 'move_file', 'apply_patch', 'apply_pending_edit', 'discard_pending_edit', 'pending_edits', 'search', 'search_files', 'semantic_index', 'index_status', 'index_build', 'index_refresh', 'index_search', 'git_context', 'git_status', 'git_diff', 'git_file_diff', 'git_log', 'git_stage', 'git_unstage', 'git_discard', 'git_commit_draft', 'execute_command', 'start_command_job', 'command_job_status', 'cancel_command_job', 'project_indexer'].includes(toolName)) {
...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        if (req.url === '/api/move_file') {
            const body = await getBody(req);
            enforcePermission(req, 'file.move', {
                targetSummary: `move_file:${String(body.sourcePath || body.path || '').slice(0, 80)}->${String(body.targetPath || '').slice(0, 80)}`
            });
            try {
                const result = await routeApiTool('move_file', {
                    sourcePath: body.sourcePath || body.path,
                    targetPath: body.targetPath,
                    reason: body.reason || 'Manual UI Move'
                });
                return sendJSON(res, 200, { ok: true, result });
            } catch (e) {
                const message = String(e.message || '');
